<!-- Header -->
<div align="center">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&size=22&duration=3000&pause=1000&color=7AA2F7&center=true&vCenter=true&width=600&lines=Hey+there%2C+I'm+Brandon+Mutua+%F0%9F%91%8B;Data+Scientist+%7C+Data+Engineer;Machine+Learning+Enthusiast;Always+learning.+Always+building.+%F0%9F%92%A1" alt="Typing SVG" />
</div>

<br/>

<div align="center">

[![Gmail](https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:mutua.brandon.data@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-BB9AF7?style=for-the-badge&logo=github&logoColor=1a1b2e)](https://github.com/brandonmutua-svg)


</div>

---

## 🧠 About Me

```python
brandon= {
    "name":       "Brandon Mutua",
    "pronouns":   "he/him",
    "roles":      ["Data Scientist", "Data Engineer","Machine Learning Enthusiast"],
    "location":   "Kenya 🇰🇪",
    "education":  "Moringa School Kenya",
    "focus":      ["ML Modeling", "ETL Pipelines", "Data Analysis",],
    "tools":      ["Python", "SQL", "Google Cloud", "TensorFlow", "Pandas"],
    "currently":  "Sharpening end-to-end data workflows & ML models",
    "motto":      "Always learning. Always building. 💡"
}
```

I'm a passionate data enthusiast with a growing foundation in Python, SQL, and data engineering. I love turning raw data into meaningful insights — and building the pipelines that make that possible.

---

## 🔍 What I'm Focused On

| Area | Description |
|------|-------------|
| 🤖 **Data Science** | ML modeling, statistical analysis, predictive insights |
| 🔧 **Data Engineering** | ETL pipelines, data ingestion, cleaning & transformation |
| 📊 **Analysis & Viz** | Python (Pandas, NumPy), Plotly, data storytelling |
| 🗄️ **SQL & Databases** | Schema design, query optimization, data modeling |
| ☁️ **Cloud** | Google Cloud — BigQuery, Dataflow, Cloud Storage |

---

## 🛠️ Tech Stack

**Languages & Core**

![Python](https://img.shields.io/badge/Python-E0AF68?style=flat-square&logo=python&logoColor=1a1b2e)
![SQL](https://img.shields.io/badge/SQL-7DCFFF?style=flat-square&logo=postgresql&logoColor=1a1b2e)

**Data & ML**

![Pandas](https://img.shields.io/badge/Pandas-73DACA?style=flat-square&logo=pandas&logoColor=1a1b2e)
![NumPy](https://img.shields.io/badge/NumPy-73DACA?style=flat-square&logo=numpy&logoColor=1a1b2e)
![Matplotlib](https://img.shields.io/badge/Matplotlib-73DACA?style=flat-square&logo=python&logoColor=1a1b2e)
![Plotly](https://img.shields.io/badge/Plotly-73DACA?style=flat-square&logo=plotly&logoColor=1a1b2e)
![SciPy](https://img.shields.io/badge/SciPy-73DACA?style=flat-square&logo=scipy&logoColor=1a1b2e)
![scikit-learn](https://img.shields.io/badge/scikit--learn-BB9AF7?style=flat-square&logo=scikit-learn&logoColor=1a1b2e)
![TensorFlow](https://img.shields.io/badge/TensorFlow-BB9AF7?style=flat-square&logo=tensorflow&logoColor=1a1b2e)
![PyTorch](https://img.shields.io/badge/PyTorch-BB9AF7?style=flat-square&logo=pytorch&logoColor=1a1b2e)
![PyShark](https://img.shields.io/badge/PyShark-F7768E?style=flat-square&logo=wireshark&logoColor=1a1b2e)

**Tools & Platforms**

![Google Cloud](https://img.shields.io/badge/Google_Cloud-7AA2F7?style=flat-square&logo=googlecloud&logoColor=1a1b2e)
![BigQuery](https://img.shields.io/badge/BigQuery-7AA2F7?style=flat-square&logo=googlebigquery&logoColor=1a1b2e)
![Apache Spark](https://img.shields.io/badge/Apache_Spark-FF9E64?style=flat-square&logo=apachespark&logoColor=1a1b2e)
![Tableau](https://img.shields.io/badge/Tableau-7DCFFF?style=flat-square&logo=tableau&logoColor=1a1b2e)
![Jupyter](https://img.shields.io/badge/Jupyter-E0AF68?style=flat-square&logo=jupyter&logoColor=1a1b2e)
![Anaconda](https://img.shields.io/badge/Anaconda-9ECE6A?style=flat-square&logo=anaconda&logoColor=1a1b2e)
![Git](https://img.shields.io/badge/Git-F7768E?style=flat-square&logo=git&logoColor=1a1b2e)
![VS Code](https://img.shields.io/badge/VS_Code-7DCFFF?style=flat-square&logo=visualstudiocode&logoColor=1a1b2e)

---

## 🌱 Currently Learning

- 🤖 Deepening ML knowledge with **TensorFlow** & **scikit-learn**
- ☁️ **Google Cloud Platform** — BigQuery, Cloud Storage, Dataflow
- 📐 **Advanced SQL** — window functions, CTEs, performance tuning
- 🔄 Building robust **ETL pipelines** with Python

---

## 📈 Contribution Graph

<div align="center">
<img src="https://github-readme-activity-graph.vercel.app/graph?username=brandonmutua&bg_color=1a1b2e&color=a9b1d6&line=7aa2f7&point=bb9af7&area=true&hide_border=true" alt="Contribution Graph" />
</div>

---

## 💬 Let's Connect!

I'm open to collaboration, data projects, and new ideas.

<div align="center">

[![Email](https://img.shields.io/badge/Email_Me-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:mutua.brandon.data@gmail.com)

</div>

---

<div align="center">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&size=13&duration=4000&pause=1000&color=565F89&center=true&vCenter=true&width=500&lines=Data+Scientist+%7C+Kenya+%F0%9F%87%B0%F0%9F%87%AA+%7C+Moringa+School" alt="footer" />
</div>
